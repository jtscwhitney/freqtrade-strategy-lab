...
# isort: skip_file
"""
AdaptiveTrend V01 Short-Only — Candidate N investigation (step 1).

Inherits V01 ROC momentum + ATR trailing. Longs disabled via empty LONG_PAIRS.
Params: MOM_LOOKBACK=24, THETA_ENTRY=0.03, ATR_MULT=3.5 — same as the +26.6% run.

Backtest (repo root, one line):
    docker compose run --rm freqtrade backtesting --config /freqtrade/config/config_adaptivetrend_shortonly.json --strategy AdaptiveTrendStrategy_V01_ShortOnly --timerange 20220101-20230101 --timeframe 6h --fee 0.0005 --cache none
"""
...
class AdaptiveTrendStrategy_V01_ShortOnly(IStrategy):
    timeframe = "6h"
    ...
    LONG_PAIRS: frozenset[str] = frozenset()  # DISABLED — short-only isolation
    ...
    MAX_CONCURRENT_LONG: int = 0
    MAX_CONCURRENT_SHORT: int = 6
